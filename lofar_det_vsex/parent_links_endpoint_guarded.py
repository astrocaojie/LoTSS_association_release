"""endpoint-guarded parent-link candidates.

endpoint-guarded keeps the production parent-linking parent association flow and only adds a hard
endpoint gate so compact or tiny-noise local groups cannot enter lobe-pair
scoring.
"""
...
def _hard_compact_veto(row: pd.Series, area3: float, axis_ratio: float, major_axis: float, minor_axis: float) -> tuple[bool, str]:
    ...
    if (
        ...
        reasons.append("single_gaussian_compact_source")
    if (
        ...
        reasons.append("observed_beam_like_source")
    return bool(reasons), ";".join(reasons)


def _noise_artifact_veto(row: pd.Series, area3: float, mask_area: float, peak_snr: float, artifact: bool, artifact_env: dict[str, Any]) -> tuple[bool, str]:
    reasons: list[str] = []
    n_gauss = safe_float(row.get("n_gaussians"), float("nan"))
    ...
    if np.isfinite(n_gauss) and int(n_gauss) == 1 and np.isfinite(mask_area) and mask_area < 1.5:
        reasons.append("single_gaussian_tiny_mask")
    if safe_float(row.get("n_only_2sigma_edges"), 0.0) > 0 and safe_float(row.get("n_strong_edges"), 0.0) <= 0:
        reasons.append("only_2sigma_evidence")
    ridge = safe_float(row.get("ridge_continuity_score"), float("nan"))
    if np.isfinite(ridge) and ridge < 0.25:
        reasons.append("low_ridge_continuity")
    if artifact or _artifact_flags(row):
        reasons.append("artifact_risk")
    return bool(reasons), ";".join(dict.fromkeys(reasons))


def _extended_evidence_count(row: pd.Series, area3: float, axis_ratio: float, major_axis: float, peak_snr: float, relaxed_axis_ratio: float = 1.6) -> int:
    atype = _association_type(row)
    allowed_types = {"continuous_extended", "diffuse_extended", "linear_or_tail_like", "complex_association", "weak_association"}
    signals = [
        safe_float(row.get("LAS_beam"), 0.0) >= 3.0,
        np.isfinite(area3) and area3 >= 2.5,
        np.isfinite(major_axis) and major_axis >= 2.0,
        axis_ratio >= relaxed_axis_ratio,
...
def _near_extended_lobe_candidate(row: pd.Series, area3: float, axis_ratio: float, major_axis: float, peak_snr: float, blocked: bool) -> tuple[bool, str]:
    if blocked:
        return False, ""
    n_gauss = safe_float(row.get("n_gaussians"), float("nan"))
    las = safe_float(row.get("LAS_beam"), float("nan"))
    evidence = _extended_evidence_count(row, area3, axis_ratio, major_axis, peak_snr, relaxed_axis_ratio=1.5)
...
def _artifact_flags(row: pd.Series) -> bool:
    ...
    tokens = ["artifact", "sidelobe", "negative_bowl", "bowl", "bright-source residual", "bright_source_residual"]
    explicit = any(bool(row.get(col, False)) for col in ["artifact_risk", "negative_bowl", "sidelobe", "bright_source_residual"])
...
def _source_morph(row: pd.Series, area3: float, mask_area: float, axis_ratio: float, major_axis: float, minor_axis: float, artifact_env: dict[str, Any], cfg: dict[str, Any]) -> dict[str, Any]:
    hard_compact, compact_reason = _hard_compact_veto(row, area3, axis_ratio, major_axis, minor_axis)
    point = _point_like(row, area3, axis_ratio, major_axis) or hard_compact
    artifact_diagnostic = safe_float(artifact_env.get("artifact_environment_score"), 0.0) >= float(cfg.get("artifact_veto_score", 1.2))
    use_artifact_layer2 = bool(cfg.get("use_artifact_penalties_layer2", True))
    artifact_effective = bool(use_artifact_layer2 and artifact_diagnostic)
    peak_snr = safe_float(row.get("peak_snr"), safe_float(row.get("peak_flux"), 0.0))
    noise_veto_diag, noise_reason = _noise_artifact_veto(row, area3, mask_area, peak_snr, artifact_diagnostic, artifact_env)
    noise_veto = bool(use_artifact_layer2 and noise_veto_diag)
    atype = _association_type(row)
    quality = _association_quality(row)
    allowed_types = {"continuous_extended", "diffuse_extended", "linear_or_tail_like", "complex_association"}
    allowed_type = atype not in {"weak_association", "artifact_risk"}
    allowed_quality = quality not in {"low", "artifact_risk"}
    lobe_signal_count = _extended_evidence_count(row, area3, axis_ratio, major_axis, peak_snr, relaxed_axis_ratio=1.6)
    reject: list[str] = []
    ...
    if artifact_effective:
        reject.append("artifact_veto")
        lobe_reject.append("artifact_veto")
    if not allowed_type:
    ...
    near_candidate, near_reason = _near_extended_lobe_candidate(row, area3, axis_ratio, major_axis, peak_snr, bool(hard_compact or noise_veto or artifact_effective or isolated_compact))
    is_lobe = bool(not hard_compact and not noise_veto and not isolated_compact and not artifact_effective and allowed_type and allowed_quality and lobe_signal_count >= 2)
    if hard_compact:
        morph = "point_like_or_compact"
    elif noise_veto:
        morph = "noise_or_artifact"
    elif isolated_compact:
        morph = "point_like_or_compact"
    elif artifact_effective:
        morph = "artifact_risk"
    elif is_lobe:
    ...
    return {
        ...
        "noise_artifact_reason": noise_reason if noise_veto_diag else "",
        ...
        "is_parent_endpoint_allowed": bool(is_lobe),
        "lobe_like_reject_reason": "" if is_lobe else (";".join(dict.fromkeys(lobe_reject)) or "not_extended_enough_for_lobe"),
        "is_artifact_risk": bool(artifact_diagnostic),
        "morph_reject_reason": ";".join(dict.fromkeys(reject)),
    }


def _postprocess_isolated_and_near_candidates(frame: pd.DataFrame, groups: pd.DataFrame, segmentation: Any, config: dict[str, Any]) -> pd.DataFrame:
    if frame.empty or groups.empty:
        return frame
    out = frame.copy()
    use_artifact_layer2 = ablation_enabled(config, "use_artifact_penalties_layer2")
    seed_by_id = {str(row["association_group_id"]): row for _, row in out.iterrows()}
    work = groups.copy().reset_index(drop=True)
    if "pixel_scale_arcsec" not in work:
        work["pixel_scale_arcsec"] = 1.5
    work = work.merge(out[["association_group_id", "robust_bbox", "robust_bbox_source", "is_parent_seed"]], on="association_group_id", how="left")
    beam_arcsec = compute_beam_size_arcsec(config)
    pixel_scale = safe_float(work["pixel_scale_arcsec"].iloc[0], 1.5)
    max_gap_pix = float(parent_link_config(config).get("max_box_gap_beam", 12.0)) * beam_arcsec / max(pixel_scale, 1e-6)
    ...
    for pair_index, (idx_i, idx_j) in enumerate(_candidate_search_pairs(work, max_gap_pix)):
        ...
        seed_i = seed_by_id.get(gid_i, pd.Series(dtype=object))
        seed_j = seed_by_id.get(gid_j, pd.Series(dtype=object))
        base = _base_pair_record(str(group_i.get("cutout_id", "")), pair_index, seed_i, seed_j, group_i, group_j, config)
        support = _pair_support_features(base, seed_i, seed_j, group_i, group_j, segmentation, config)
        if bool(support.get("same_3sigma_region_as_neighbor", False)) or bool(support.get("same_2p5sigma_region_as_neighbor", False)) or bool(support.get("bridge_snr_support", False)) or safe_float(support.get("ridge_continuity_score_pair"), 0.0) >= 0.5:
            connected_ids.update([gid_i, gid_j])
    for idx, row in out.iterrows():
        gid = str(row.get("association_group_id"))
        if bool(row.get("isolated_compact_veto", False)) and gid in connected_ids:
            out.at[idx, "isolated_compact_veto"] = False
            out.at[idx, "isolated_compact_reason"] = ""
            artifact_blocked = use_artifact_layer2 and bool(row.get("is_artifact_risk", False))
            if not bool(row.get("hard_compact_veto", False)) and not bool(row.get("noise_artifact_veto", False)) and not artifact_blocked and str(row.get("source_morph_class")) == "point_like_or_compact":
                out.at[idx, "source_morph_class"] = "resolved_single"
                out.at[idx, "is_point_like"] = False
        blocked = (
            bool(out.at[idx, "hard_compact_veto"])
            or bool(out.at[idx, "noise_artifact_veto"])
            or bool(out.at[idx, "isolated_compact_veto"])
            or (use_artifact_layer2 and bool(out.at[idx, "is_artifact_risk"]))
        )
        near, reason = _near_extended_lobe_candidate(out.loc[idx], safe_float(out.at[idx, "area_3sigma_beam"], float("nan")), safe_float(out.at[idx, "axis_ratio"], 1.0), safe_float(out.at[idx, "major_axis_beam"], float("nan")), safe_float(out.at[idx, "peak_snr"], 0.0), blocked)
        out.at[idx, "near_extended_lobe_candidate"] = bool(near)
        out.at[idx, "near_lobe_rescue_reason"] = reason
    return out
...
def build_source_morph_table(
    ...
    frame = _postprocess_isolated_and_near_candidates(frame, groups, segmentation, config)
    return _with_columns(frame, SOURCE_MORPH_TABLE_COLUMNS)
...
def _bbox_gap_pix(box_a: tuple[float, float, float, float] | None, box_b: tuple[float, float, float, float] | None) -> float:
    if box_a is None or box_b is None:
        return float("nan")
    dx = max(box_a[0] - box_b[2] - 1.0, box_b[0] - box_a[2] - 1.0, 0.0)
    dy = max(box_a[1] - box_b[3] - 1.0, box_b[1] - box_a[3] - 1.0, 0.0)
    return float(np.hypot(dx, dy))


def _base_pair_record(cutout_id: str, pair_index: int, seed_a: pd.Series, seed_b: pd.Series, group_a: pd.Series, group_b: pd.Series, config: dict[str, Any]) -> dict[str, Any]:
    box_a = _bbox_tuple(seed_a.get("robust_bbox", "")) or _bbox_tuple(group_a.get("bounding_box", ""))
    box_b = _bbox_tuple(seed_b.get("robust_bbox", "")) or _bbox_tuple(group_b.get("bounding_box", ""))
    pixel_scale = safe_float(group_a.get("pixel_scale_arcsec"), safe_float(group_b.get("pixel_scale_arcsec"), 1.5))
    beam_arcsec = compute_beam_size_arcsec(config)
    gap_pix = _bbox_gap_pix(box_a, box_b)
    gap_arcsec = gap_pix * pixel_scale if np.isfinite(gap_pix) else float("nan")
    gap_beam = gap_arcsec / max(beam_arcsec, 1e-6) if np.isfinite(gap_arcsec) else float("nan")
    return {
        "cutout_id": cutout_id,
        "parent_candidate_id": f"{cutout_id}_eg_pc{pair_index:04d}",
        "parent_candidate_type": "endpoint_guarded_endpoint_gate",
        "local_group_id_1": str(group_a.get("association_group_id")),
        "local_group_id_2": str(group_b.get("association_group_id")),
        "box_gap_pix": gap_pix,
        "box_gap_arcsec": gap_arcsec,
        "box_gap_beam_raw": gap_beam,
        "box_gap_beam_robust": gap_beam,
        "box_gap_source": "endpoint_guarded_bbox",
    }
...
def _threshold_index(segmentation: Any, threshold: float) -> int | None:
    ...
    if thresholds.size == 0:
        return None
    return int(np.argmin(np.abs(thresholds - float(threshold))))
...
def _label_at_group(segmentation: Any, group: pd.Series, threshold: float) -> int:
    ...
    h, w = labels.shape
    x = int(round(np.clip(safe_float(group.get("centroid_x"), -1), 0, w - 1)))
    y = int(round(np.clip(safe_float(group.get("centroid_y"), -1), 0, h - 1)))
    if x < 0 or y < 0:
        return 0
...
def _pair_support_features(
    ...
    near_boundary = bool(np.isfinite(gap) and gap >= 0 and ((np.isfinite(gap_mean) and gap_mean <= 1.0) or (gap <= 6.0 and np.isfinite(gap_min) and gap_min <= 1.5)))
...
def _near_boundary_strong_support(record: dict[str, Any], seed_a: pd.Series, seed_b: pd.Series) -> bool:
    n1 = safe_float(seed_a.get("n_gaussians"), 0.0)
    n2 = safe_float(seed_b.get("n_gaussians"), 0.0)
    area1 = safe_float(seed_a.get("area_3sigma_beam"), 0.0)
    area2 = safe_float(seed_b.get("area_3sigma_beam"), 0.0)
    return bool(
        bool(record.get("same_3sigma_region_as_neighbor", False))
        or bool(record.get("same_2p5sigma_region_as_neighbor", False))
        or safe_float(record.get("ridge_continuity_score_pair"), 0.0) >= 0.5
        or bool(record.get("bridge_snr_support", False))
        or (n1 >= 2 and n2 >= 2 and max(n1, n2) >= 3)
        or (area1 >= 2.5 and area2 >= 2.5)
    )


def _normal_endpoint_allowed(seed_a: pd.Series, seed_b: pd.Series) -> bool:
    return bool(
        ...
        and bool(seed_a.get("is_parent_endpoint_allowed", False))
        and bool(seed_b.get("is_parent_endpoint_allowed", False))
        and not bool(seed_a.get("hard_compact_veto", False))
        and not bool(seed_b.get("hard_compact_veto", False))
        and not bool(seed_a.get("noise_artifact_veto", False))
...
def _rescue_endpoint_allowed(record: dict[str, Any], seed_a: pd.Series, seed_b: pd.Series, use_artifact_layer2: bool = True) -> bool:
    return bool(
        bool(record.get("near_boundary_pair", False))
        and bool(seed_a.get("near_extended_lobe_candidate", False))
        and bool(seed_b.get("near_extended_lobe_candidate", False))
        and not bool(seed_a.get("hard_compact_veto", False))
        and not bool(seed_b.get("hard_compact_veto", False))
        and not bool(seed_a.get("noise_artifact_veto", False))
        and not bool(seed_b.get("noise_artifact_veto", False))
        and not bool(seed_a.get("isolated_compact_veto", False))
        and not bool(seed_b.get("isolated_compact_veto", False))
        and not (use_artifact_layer2 and bool(seed_a.get("is_artifact_risk", False)))
        and not (use_artifact_layer2 and bool(seed_b.get("is_artifact_risk", False)))
    )


def _classify_pair(record: dict[str, Any], seed_a: pd.Series, seed_b: pd.Series, cfg: dict[str, Any], rescue_allowed: bool = False) -> tuple[bool, str]:
    ...
    if use_endpoint_filtering and not _normal_endpoint_allowed(seed_a, seed_b) and not rescue_allowed:
        return False, "endpoint_not_lobe_candidate"
    if bool(cfg.get("use_artifact_penalties_layer2", True)) and (class_a == "artifact_risk" or class_b == "artifact_risk"):
        return False, "artifact_veto"
    if use_endpoint_filtering and (class_a == "point_like" or class_b == "point_like"):
        if class_a != class_b:
            return False, "asymmetric_or_point_lobe_pair"
        return False, "point_like_endpoint"
    if use_endpoint_filtering and (class_a != "lobe_candidate" or class_b != "lobe_candidate"):
        return False, "asymmetric_or_point_lobe_pair"
...
def _classify_near_boundary_rescue(record: dict[str, Any], seed_a: pd.Series, seed_b: pd.Series, cfg: dict[str, Any]) -> tuple[bool, str]:
    ...
    if safe_float(record.get("axis_alignment_score"), 0.0) < 0.55:
        return False, "near_boundary_axis_alignment_too_low"
    if safe_float(record.get("facing_score"), 0.0) < 0.45:
        return False, "near_boundary_facing_too_low"
    if bool(cfg.get("use_stage2_relative_scale_constraints", True)):
...
def _make_pair_edges(cutout_id: str, groups: pd.DataFrame, morph: pd.DataFrame, segmentation: Any, config: dict[str, Any]) -> pd.DataFrame:
    ...
    beam_arcsec = compute_beam_size_arcsec(config)
    pixel_scale = safe_float(work["pixel_scale_arcsec"].iloc[0], 1.5)
    max_gap_pix = float(cfg.get("max_box_gap_beam", 12.0)) * beam_arcsec / max(pixel_scale, 1e-6)
    records: list[dict[str, Any]] = []
    for pair_index, (idx_i, idx_j) in enumerate(_candidate_search_pairs(work, max_gap_pix)):
        group_i = work.iloc[idx_i]
        group_j = work.iloc[idx_j]
        ...
        base_record = _base_pair_record(cutout_id, pair_index, seed_i, seed_j, group_i, group_j, config)
        base_record.update(_pair_support_features(base_record, seed_i, seed_j, group_i, group_j, segmentation, config))
        use_endpoint_filtering = bool(cfg.get("use_stage2_endpoint_filtering", True))
        normal_endpoint = True if not use_endpoint_filtering else _normal_endpoint_allowed(seed_i, seed_j)
        rescue_endpoint = False if not use_endpoint_filtering else _rescue_endpoint_allowed(base_record, seed_i, seed_j, bool(cfg.get("use_artifact_penalties_layer2", True)))
        if not normal_endpoint and not rescue_endpoint:
            record = {
                **base_record,
                "double_lobe_geometry_pass": False,
                "near_boundary_rescue_applied": False,
                "near_boundary_rescue_reason": "",
                "parent_candidate_quality": "rejected",
                "host_status": "not_queried",
                "host_quality": "none",
                "host_evidence": "not_checked",
                "rejection_reason": "endpoint_not_lobe_candidate",
                ...
                "n_gaussians_1": seed_i.get("n_gaussians", np.nan),
                "n_gaussians_2": seed_j.get("n_gaussians", np.nan),
                "LAS_beam_1": seed_i.get("LAS_beam", np.nan),
                "LAS_beam_2": seed_j.get("LAS_beam", np.nan),
                "mask_area_beam_1": seed_i.get("mask_area_beam", np.nan),
                "mask_area_beam_2": seed_j.get("mask_area_beam", np.nan),
                "area_3sigma_beam_1": seed_i.get("area_3sigma_beam", np.nan),
                "area_3sigma_beam_2": seed_j.get("area_3sigma_beam", np.nan),
                "peak_snr_1": seed_i.get("peak_snr", np.nan),
                "peak_snr_2": seed_j.get("peak_snr", np.nan),
                "debug_info": json_dumps_safe(
                    {
                        ...
                        "lobe_like_reject_reason_1": seed_i.get("lobe_like_reject_reason", ""),
                        "lobe_like_reject_reason_2": seed_j.get("lobe_like_reject_reason", ""),
                    }
                ),
            }
        ...
        record.update(_pair_support_features(record, seed_i, seed_j, group_i, group_j, segmentation, config))
        ...
        normal_pass, normal_reason = _classify_pair(record, seed_i, seed_j, cfg, rescue_allowed=False)
        rescue_pass, rescue_reason = (False, "")
        rescue_applied = False
        if rescue_endpoint and not normal_pass:
            rescue_pass, rescue_reason = _classify_near_boundary_rescue(record, seed_i, seed_j, cfg)
            rescue_applied = bool(rescue_pass)
        geom_pass = bool(normal_pass or rescue_pass)
        reason = "" if geom_pass else ((rescue_reason or normal_reason) if rescue_endpoint else normal_reason)
        artifact_pair = max(safe_float(seed_i.get("artifact_environment_score"), 0.0), safe_float(seed_j.get("artifact_environment_score"), 0.0))
        lobe_score = float(
            1.4 * symmetry
        ...
        record.update(
            {
                "parent_candidate_id": f"{cutout_id}_eg_pc{pair_index:04d}",
                "parent_candidate_type": "endpoint_guarded_near_boundary_rescue" if rescue_applied else "endpoint_guarded_double_lobe",
...
def run_parent_links_endpoint_guarded(
    cutout_id: str,
    ...
    cfg = parent_link_config(config)
    use_artifact_layer2 = ablation_enabled(config, "use_artifact_penalties_layer2")
    use_midpoint_support = ablation_enabled(config, "use_midpoint_host_support")
    use_lobe_peak_contradiction = ablation_enabled(config, "use_lobe_peak_host_contradiction")
    ...
    if not edges.empty:
        ...
        for idx, edge in pass_edges.iterrows():
            ...
            midpoint_support_diagnostic = _quality_rank(host_quality) >= _quality_rank("medium")
            midpoint_support = bool(use_midpoint_support and midpoint_support_diagnostic)
            contradiction_lobe_peak_found = bool(use_lobe_peak_contradiction and lobe_peak_found)
            rejection = ""
            needs = True
            if use_lobe_peak_contradiction and lobe1_found and lobe2_found:
                host_evidence = "likely_independent_sources"
                quality = "rejected"
                rejection = "likely_independent_sources"
            elif contradiction_lobe_peak_found and not midpoint_support:
                host_evidence = "contradicts_double_lobe"
                quality = "rejected"
                rejection = "lobe_peak_host_contradiction"
            elif midpoint_support and not contradiction_lobe_peak_found:
                host_evidence = "supports_double_lobe"
                artifact_pair = safe_float(parent.get("artifact_environment_score_pair"), 0.0)
                if use_artifact_layer2 and artifact_pair >= float(cfg.get("artifact_suspicious_score", 0.8)):
                    quality = "suspicious"
                else:
                    quality = "high" if host_quality == "high" and safe_float(parent.get("symmetry_score"), 0.0) >= 0.70 else "medium"
            elif midpoint_support and contradiction_lobe_peak_found:
            ...
            if bool(parent.get("near_boundary_rescue_applied", False)):
                if quality == "high":
                    quality = "medium"
                if host_evidence == "needs_host_check" and safe_float(parent.get("symmetry_score"), 0.0) < 0.68:
                    quality = "suspicious"
                    suspicious_reason = "near_boundary_rescue_low_confidence"
            host_score = safe_float(best.get("host_score"), 0.0)
            final_score = safe_float(parent.get("lobe_pair_score"), 0.0) + (
                float(host_cfg.get("host_score_weight", 1.0)) * host_score if use_midpoint_support else 0.0
            )
            updates = {
                ...
                "host_status": "host_found" if midpoint_support_diagnostic else "host_query_failed" if query_failed else "no_plausible_host",
    ...
    candidate_mask = edges["parent_candidate_quality"].astype(str).isin(["high", "medium", "needs_host_check", "suspicious"]) if not edges.empty else pd.Series(dtype=bool)
...
run_parent_links = run_parent_links_endpoint_guarded
...
def _diagnostics(cutout_id: str, morph: pd.DataFrame, edges: pd.DataFrame, query_log: pd.DataFrame) -> pd.DataFrame:
    ...
    hard = morph.get("hard_compact_veto", pd.Series(dtype=bool)).astype(bool) if not morph.empty else pd.Series(dtype=bool)
    ...
    if not edges.empty:
        ...
        edge_isolated = edges.get("endpoint1_isolated_compact_veto", pd.Series(False, index=edges.index)).astype(bool) | edges.get("endpoint2_isolated_compact_veto", pd.Series(False, index=edges.index)).astype(bool)
        ...
        edge_isolated = pd.Series(dtype=bool)
            ...
            [
                {
                    ...
                    "n_hard_compact_veto": int(hard.sum()) if not morph.empty else 0,
                    ...
                    "n_point_like_hidden": int(mclass.isin(["point_like", "point_like_or_compact"]).sum()),
                    ...
                    "n_artifact_risk": int((mclass == "artifact_risk").sum()),
                    ...
                    "n_final_candidates": int(quality.isin(["high", "medium", "needs_host_check", "suspicious"]).sum()),
                    ...
                    "n_compact_endpoint_rejected": int(((reason == "endpoint_not_lobe_candidate") & edge_compact).sum()) if not edges.empty else 0,
                    "n_noise_artifact_endpoint_rejected": int(((reason == "endpoint_not_lobe_candidate") & edge_noise).sum()) if not edges.empty else 0,
                    "n_endpoint_not_lobe_candidate": int((reason == "endpoint_not_lobe_candidate").sum()),
                    ...
                    "n_geometry_pass_isolated_compact_endpoint": int((geometry_pass & edge_isolated).sum()) if not edges.empty else 0,
                    ...
                    "n_parent_candidate_isolated_compact_endpoint": int((final_mask & edge_isolated).sum()) if not edges.empty else 0,
                    "n_rejected_point_like_endpoint": int((reason == "endpoint_not_lobe_candidate").sum()),
